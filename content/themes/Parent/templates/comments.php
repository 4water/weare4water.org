...
?>
		<section id="comments">
			...
			<?php if (!comments_open() && !is_page() && post_type_supports(get_post_type(), 'comments')) : ?>
				<div class="alert"><?php _e('Comments are closed.', 'roots'); ?></div>
			<?php endif; ?>
		</section><!-- /#comments -->
<?php endif; ?>
	...
	<section id="respond">
		...
		<?php else : ?>
			<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
				...
				<?php else : ?>
					<label for="author">
						<?php _e('Name', 'roots'); if ($req) _e(' (required)', 'roots'); ?>
						<input type="text" placeholder="<?php _e('Name', 'roots');?>" class="text" name="author" id="author" value="<?php echo esc_attr($comment_author); ?>" size="22" <?php if ($req) echo 'aria-required="true"'; ?>>
					</label>
					<label for="email">
						<?php _e('Email (will not be published)', 'roots'); if ($req) _e(' (required)', 'roots'); ?>
						<input type="email" placeholder="<?php _e('Email (will not be published)', 'roots'); ?>" class="text" name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>" size="22" <?php if ($req) echo 'aria-required="true"'; ?>>
					</label>
					<label for="url">
						<?php _e('Website', 'roots'); ?>
						<input type="url" placeholder="<?php _e('Website', 'roots'); ?>" class="text" name="url" id="url" value="<?php echo esc_attr($comment_author_url); ?>" size="22">
					</label>
				<?php endif; ?>
				<label for="comment"><?php _e('Comment', 'roots'); ?>
