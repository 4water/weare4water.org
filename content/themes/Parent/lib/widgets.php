...
class ft_Recent_Comments_Widget extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_ft_recent_comments', 'description' => __('Use this widget to display recent comments', 'ft'));

	    $this->WP_Widget('widget_ft_recent_comments', __('Advanced Recent Comments', 'ft'), $widget_ops);
	    $this->alt_option_name = 'widget_roots_vcard';
	...
	function widget($args, $instance) {
		...
		$comm = '<section class="widget_recent_comments"><div class="widget-inner"><h3>Recent Comments</h3>';
		if ( $comments ) : 
			$comm .='<ul>';
			foreach ( $comments as $comment ) :
//				print_r($comment);
				$comm .= '<li>';
				$comm .= '<p><a href="/?p='.$comment->comment_post_ID.'">&quot;' . strip_tags( apply_filters( 'get_comment_text', $comment->comment_content ) ) . '&quot;</a></p>';
				$comm .= '<span>'.ft_nice_date($comment->comment_date).', <em>by</em> '.$comment->comment_author.'</span>';
				$comm .= '</li>';
			endforeach; 
			$comm .='</ul>';
		else :
